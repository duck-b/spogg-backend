...
router.get('/', function(req, res, next) {
  pool.getConnection((err, conn) => {
    ...
    const query = `SELECT notice_no AS id, notice_title, created_at FROM s_notice ORDER BY created_at DESC`;
...
router.get('/:noticeNo', function(req, res, next) {
  pool.getConnection((err, conn) => {
    if (err) {
      throw err;
    }
    const query = `SELECT * FROM s_notice WHERE notice_no = ?;`;
    conn.query(query, [req.params.noticeNo], (e, rows) => {
      if (e) throw e;
      res.send(rows);
    })
    conn.release();
...
router.post('/create', function(req, res, next) {
  pool.getConnection((err, conn) => {
    ...
    const query = `INSERT INTO s_notice (notice_title, notice_content) VALUES (?, ?);`;
    const { title, content } = req.body;
    conn.query(query, [title, content], (e, row) => {
      if (e) throw e;
      res.send({result: row.affectedRows});
    })
    conn.release();
