  ...
  pool.getConnection((err, conn) => {
    ...
    if(sess[userKey]){
      ...
      for(let i = 0; i<sport_no.length; i++){
        if(i !== 0){
          backQuery += ` OR bpv.sport_no = ${sport_no[i]}`;
        }else{
          backQuery += `AND  (bpv.sport_no = ${sport_no[i]}`;
        }
      }
      backQuery += `)`
      if(search){
      ...
      query = `SELECT DISTINCT bpv.*, uf.created_at AS user_follow, (CASE WHEN bpv.user_no = ${sess[userKey].user_no} THEN '1' ELSE '0' END) AS this_user
                FROM s_boardPlayView AS bpv 
                LEFT JOIN s_userFollower AS uf
                ON uf.userFollow_follow = bpv.user_no AND uf.userFollow_follower = ${sess[userKey].user_no} 
                WHERE deleted_at IS NULL ${backQuery} 
                ORDER BY bpv.created_at DESC;`
      }else{
        if(search){
          backQuery += ` AND (user_name = '${search}' OR board_hashtag LIKE '%${search},%')`
        }
        query = `SELECT DISTINCT *, null AS user_follow, '0' AS this_user FROM s_boardPlayView WHERE deleted_at IS NULL ${backQuery} ORDER BY created_at DESC;`;
      }
      conn.query(query, (e, rows) => {
        ...
        if(sess[userKey]){
          ...
          let boardKeywordQuery = `SELECT sportDetail_keyword FROM s_sportDetail WHERE sportDetail_page = 2 ${boardKeywordBackQuery} ORDER BY sportDetail_no;`
...
router.get('/:userNo', function(req, res, next) {
  pool.getConnection((err, conn) => {
    ...
    let query = `SELECT DISTINCT * FROM s_boardPlayView WHERE deleted_at IS NULL AND user_no = ${userNo} ORDER BY created_at DESC;`;
    conn.query(query, (e, rows) => {
      if (e) throw e;
        res.send({values: rows});
      })
    ...
    const query = `SELECT a.*, b.comment_count, (CASE WHEN a.user_no = ${user_no} THEN 1 ELSE NULL END) AS write_user
                    FROM(SELECT a.*, uf.created_at AS user_follow 
                      FROM (SELECT a.*, ub.created_at AS user_save 
                        FROM (SELECT bpv.*, ul.created_at AS user_like 
                          FROM s_boardPlayView AS bpv 
                          LEFT JOIN s_userLike AS ul 
                          ON bpv.board_no = ul.board_no AND ul.user_no = ${user_no} 
                        WHERE bpv.board_no = ${boardId}) AS a 
  ...
  if(sess[userKey]){
    pool.getConnection((err, conn) => {
      ...
      const query = `BEGIN;
        ...
        SELECT DISTINCT * FROM s_boardPlayView WHERE board_no = ${req.params.boardId} AND user_no = ${sess[userKey].user_no} AND deleted_at IS NULL;
      COMMIT;`;
      conn.query(query, (e, rows) => {
        if (e) throw e;
        if(rows[2][0]){
    ...
    pool.getConnection((err, conn) => {
      ...
      for(i = 0; i<boardDetailNo.length; i++){
        if(boardDetailNo[i] !== 'undefined'){
          query += `\nUPDATE s_boardDetail SET board_no = ?, boardDetail_num = ?, boardDetail_content = ? WHERE boardDetail_no = ?;`;
          queryData.push(boardNo, i+1, boardContent[i], boardDetailNo[i]);
          if(changeImg[i] === '1'){
...
/*
스포츠 활동 뷰 생성
CREATE ALTER VIEW s_boardPlayView AS 
...
FROM (SELECT a.*, g.goods_name, g.goods_manufacturer
	FROM (SELECT a.*, bg.boardGoods_info, bg.boardGoods_x, bg.boardGoods_y, bg.goods_no
		FROM (SELECT a.*,
			...
			WHERE b.board_kind = 1 AND b.sport_no = s.sport_no) AS a
...
/이거 부터
SELECT a.*, uf.created_at AS user_follow FROM (SELECT a.*, ub.created_at AS user_save  FROM (SELECT bpv.*, ul.created_at AS user_like   FROM s_boardPlayView AS bpv    LEFT JOIN s_userLike AS ul   ON bpv.board_no = ul.board_no AND ul.user_no = 59   WHERE bpv.board_no = 20) AS a  LEFT JOIN s_userBoard AS ub  ON a.board_no = ub.board_no AND ub.user_no = 59) AS a LEFT JOIN s_userFollower AS uf ON a.user_no = uf.userFollow_follow AND uf.userFollow_follower = 59 LIMIT 0, 500
*/